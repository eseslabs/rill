...
export function buildSkillDoc(skill: PublishedSkill): string {
  ...
  return [
    ...
    '```',
    '',
    'OpenCode — add both to `opencode.json` instead:',
    '',
    '```json',
    '{',
    '  "$schema": "https://opencode.ai/config.json",',
    '  "mcp": {',
    `    "rill-actions": { "type": "remote", "url": "${mcpUrl}", "enabled": true },`,
    `    "rill-wallet": { "type": "local", "command": ["./rill-wallet"], "environment": { "SUI_NETWORK": "${config.network}" }, "enabled": true }`,
    '  }',
    '}',
    '```',
    '',
    'Launch Claude/OpenCode from the shell where `RILL_SUI_PRIVATE_KEY` is already set so the local server inherits it',
