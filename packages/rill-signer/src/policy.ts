...
type AnyCommand = ReturnType<Transaction['getData']>['commands'][number];
type AnyInput = ReturnType<Transaction['getData']>['inputs'][number];

function targetOf(command: AnyCommand): string {
  return command.$kind === 'MoveCall'
    ? `${normalized(command.MoveCall.package)}::${command.MoveCall.module}::${command.MoveCall.function}`
    : '';
}

function inputAt(data: ReturnType<Transaction['getData']>, argument: unknown, label: string): AnyInput {
...
function objectIdFromInput(input: AnyInput): string | undefined {
...
function objectArgument(data: ReturnType<Transaction['getData']>, argument: unknown, label: string): string {
  const objectId = objectIdFromInput(inputAt(data, argument, label));
  if (!objectId) throw new Error(`${label} is not an object input.`);
  return objectId;
}

function pureBytes(data: ReturnType<Transaction['getData']>, argument: unknown, label: string): Buffer {
  const input = inputAt(data, argument, label);
  if (input.$kind !== 'Pure') throw new Error(`${label} is not pure.`);
  return Buffer.from(input.Pure.bytes, 'base64');
}

function u64(data: ReturnType<Transaction['getData']>, argument: unknown, label: string): bigint {
  const bytes = pureBytes(data, argument, label);
  if (bytes.length !== 8) throw new Error(`${label} is not a u64.`);
  return bytes.readBigUInt64LE();
}

function byte(data: ReturnType<Transaction['getData']>, argument: unknown, label: string): number {
  const bytes = pureBytes(data, argument, label);
  if (bytes.length !== 1) throw new Error(`${label} is not a byte.`);
  return bytes[0];
}

function exactArity(arguments_: readonly unknown[], expected: number, label: string) {
...
function inspect(transaction: Transaction, policy: LocalSignerPolicy) {
  ...
  const callTargets = data.commands.flatMap((command) => {
    const target = targetOf(command);
    return target ? [target] : [];
  });
  const uniqueTargets = [...new Set(callTargets)];
  ...
  if (policy.allowedTargets.some((t) => normalizeTarget(t) === walletTarget)) {
    const hasSpend = uniqueTargets.includes(walletTarget);
    if (!hasSpend) throw new Error('PTB is missing required wallet spend target.');
  }

  const policyTargets = policy.allowedTargets.map(normalizeTarget);
  const policyTargetSet = new Set(policyTargets);
  const offScope = uniqueTargets.find((target) => !policyTargetSet.has(target));
  if (offScope) throw new Error(`PTB contains off-scope target ${offScope}.`);
  if (!sameValues(uniqueTargets, policyTargets)) {
    throw new Error('PTB target manifest mismatch.');
  }
  if (
    callTargets.length !== policyTargets.length ||
    !callTargets.every((target, index) => target === policyTargets[index])
  ) {
  ...
  for (const guard of requiredGuards) {
    if (!uniqueTargets.includes(guard)) throw new Error(`PTB is missing required guard ${guard}.`);
  }

  const spendIndex = data.commands.findIndex((command) => targetOf(command) === walletTarget);
  let spendAmountMist = 0n;

  if (spendIndex >= 0) {
    const spend = data.commands[spendIndex];
    if (spend.$kind !== 'MoveCall') throw new Error('PTB wallet spend is not a MoveCall.');
    exactArity(spend.MoveCall.arguments, 4, 'PTB wallet spend');
    expectNormalizedMatch(
      objectArgument(data, spend.MoveCall.arguments[3], 'wallet spend clock'),
      '0x6',
      'PTB wallet spend clock mismatch.',
    );
    expectNormalizedMatch(
      objectArgument(data, spend.MoveCall.arguments[0], 'wallet spend wallet'),
      policy.walletId,
      'PTB wallet spend walletId mismatch.',
    );
    expectNormalizedMatch(
      objectArgument(data, spend.MoveCall.arguments[1], 'wallet spend AgentCap'),
      policy.agentCapId,
      'PTB wallet spend agentCapId mismatch.',
    );
    spendAmountMist = u64(data, spend.MoveCall.arguments[2], 'wallet spend amount');

    const mergeCommands = data.commands.filter((command) => command.$kind === 'MergeCoins');
    if (mergeCommands.length !== 1) {
      throw new Error('PTB must contain exactly one MergeCoins cleanup for wallet funding.');
    }
    const merge = mergeCommands[0];
    if (merge.$kind !== 'MergeCoins') throw new Error('Unreachable.');
    const cleanupSource = merge.MergeCoins.sources[0];
    const mergeIdx = data.commands.findIndex((c) => c === merge);
    if (
      mergeIdx <= spendIndex ||
      mergeIdx !== data.commands.length - 1 ||
  ...
  // DeepBook-specific verification (backward compatible with existing policies).
  if (policy.balanceManagerId && policy.tradeCapId && policy.poolId && policy.onChainOrder) {
    ...
    expectNormalizedMatch(
      objectArgument(data, deposit.MoveCall.arguments[0], 'DeepBook deposit BalanceManager'),
    ...
    const depositAmountMist = u64(data, split.SplitCoins.amounts[0], 'DeepBook deposit amount');
    ...
    expectNormalizedMatch(
      objectArgument(data, proof.MoveCall.arguments[0], 'DeepBook proof BalanceManager'),
      policy.balanceManagerId,
      'PTB DeepBook proof BalanceManager mismatch.',
    );
    expectNormalizedMatch(
      objectArgument(data, proof.MoveCall.arguments[1], 'DeepBook proof TradeCap'),
      policy.tradeCapId,
    ...
    expectNormalizedMatch(
      objectArgument(data, order.MoveCall.arguments[11], 'DeepBook order clock'),
      '0x6',
      'PTB DeepBook order clock mismatch.',
    );
    expectNormalizedMatch(
      objectArgument(data, order.MoveCall.arguments[0], 'DeepBook order pool'),
      policy.poolId,
      'PTB DeepBook order poolId mismatch.',
    );
    expectNormalizedMatch(
      objectArgument(data, order.MoveCall.arguments[1], 'DeepBook order BalanceManager'),
      policy.balanceManagerId,
    ...
    const isBid = byte(data, order.MoveCall.arguments[8], 'DeepBook order isBid');
    const payWithDeep = byte(data, order.MoveCall.arguments[9], 'DeepBook order payWithDeep');
    if (isBid > 1 || payWithDeep > 1) throw new Error('PTB DeepBook order contains an invalid boolean.');
    const onChainOrder = {
      clientOrderId: u64(data, order.MoveCall.arguments[3], 'DeepBook order clientOrderId').toString(),
      orderType: String(byte(data, order.MoveCall.arguments[4], 'DeepBook order orderType')),
      selfMatchingOption: String(byte(data, order.MoveCall.arguments[5], 'DeepBook order selfMatchingOption')),
      price: u64(data, order.MoveCall.arguments[6], 'DeepBook order price').toString(),
      quantity: u64(data, order.MoveCall.arguments[7], 'DeepBook order quantity').toString(),
      isBid: isBid === 1,
      payWithDeep: payWithDeep === 1,
      expiration: u64(data, order.MoveCall.arguments[10], 'DeepBook order expiration').toString(),
    };
  ...
  return { data, targets: uniqueTargets, callTargets, objectIds, spendAmountMist };
}
...
export async function validateExecutionEnvelope(
  ...
  const identityPairs: Array<[string, string | undefined, string | undefined]> = [
  ...
  for (const [name, actual, expected] of identityPairs) {
    ...
    if (actual === undefined || normalized(actual) !== normalized(expected)) {
      throw new Error(`ExecutionEnvelope ${name} mismatch.`);
    }
  }

  if (policy.poolId && normalized(String(envelope.resolvedParams.poolId ?? '')) !== normalized(policy.poolId)) {
  ...
  const policyTargetSet = new Set(policy.allowedTargets.map(normalizeTarget));
  const offScope = inspected.targets.find((target) => !policyTargetSet.has(target));
  if (offScope) throw new Error(`PTB contains off-scope target ${offScope}.`);
  if (
    inspected.callTargets.length !== policyTargetSet.size ||
    !inspected.callTargets.every((target, index) => target === [...policyTargetSet][index])
  ) {
    throw new Error('PTB target sequence differs from the local policy.');
  }
  for (const guard of requiredGuards) {
  ...
  if (!sameValues(inspected.objectIds, envelopeObjects)) {
    throw new Error('PTB requiredObjectIds object policy mismatch.');
  }
  const policyObjectIds = policy.requiredObjectIds.map(normalized);
  if (!sameValues(inspected.objectIds, policyObjectIds)) {
    throw new Error('PTB requiredObjectIds object policy mismatch.');
  }

  const resolvedSpend = decimalU64(
    String(envelope.resolvedParams.spendAmountMist ?? '0'),
  ...
  // DeepBook-specific amount check (backward compatible).
  if (policy.demoParams) {
...
export async function assertCapabilitiesActive(
  ...
  const capabilities: Array<[string, string, string, string]> = [
    ['AgentCap', policy.agentCapId, 'wallet', policy.walletId],
  ];
  if (policy.tradeCapId && policy.balanceManagerId) {
